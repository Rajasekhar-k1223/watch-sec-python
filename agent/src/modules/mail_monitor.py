...
class MailMonitorStrategy:
    def __init__(self, api_url, agent_id, api_key, data_queue=None):
...
class WindowsOutlookStrategy(MailMonitorStrategy):
    def __init__(self, api_url, agent_id, api_key, data_queue=None):
        super().__init__(api_url, agent_id, api_key, data_queue=data_queue)
    def _loop(self):
        try:
...
class LinuxThunderbirdStrategy(MailMonitorStrategy):
    ...
    def _loop(self):
        ...
        import email.utils # type: ignore
...
# --- Facade ---
class MailMonitor:
    def __init__(self, backend_url, agent_id, api_key, data_queue=None):
        self.strategy = None
        os_type = platform.system()
        
        if os_type == "Windows":
            self.strategy = WindowsOutlookStrategy(backend_url, agent_id, api_key, data_queue=data_queue)
        elif os_type == "Linux":
            self.strategy = LinuxThunderbirdStrategy(backend_url, agent_id, api_key, data_queue=data_queue)
        elif os_type == "Darwin":
            self.strategy = MacAppleMailStrategy(backend_url, agent_id, api_key, data_queue=data_queue)
        else:
