...
# --- Public Endpoints (No Auth Header, uses Query Param) ---
...
async def download_public_agent(
    ...
    # 2. Reuse Logic (Copy-Paste or Refactor? Refactor is cleaner, but copy is safer for now to avoid breaking existing auth flow)
    # ... (Logic identical to download_agent but using 'tenant' from key)
...
async def get_install_script(request: Request, key: str):
    ...
    download_url = f"{base_url}/api/downloads/public/agent?key={key}&os_type=windows"
    
    ps_script = f"""
$ErrorActionPreference = 'Stop'
Write-Host "--- WatchSec Installer ---" -ForegroundColor Cyan
$Url = "{download_url}"
$Dest = "$env:TEMP\\watch-sec-installer.exe"
